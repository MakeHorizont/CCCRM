...
const UserProfileAttendanceTab: React.FC<UserProfileAttendanceTabProps> = ({
  ...
  const [isClearAllMonthConfirmOpen, setIsClearAllMonthConfirmOpen] = useState(false);
  ...
  const [isClearMultiSelectedConfirmOpen, setIsClearMultiSelectedConfirmOpen] = useState(false);
  ...
  useEffect(() => {
    ...
    setIsClearAllMonthConfirmOpen(false);
    setIsFill52ConfirmOpen(false);
    setIsClearMultiSelectedConfirmOpen(false);
  ...
  const handleBulkFillWeekdays = (fillType: AttendanceEntry['type'], overwrite: boolean) => {
    ...
    for (let day = 1; day <= daysInMonth; day++) {
        // FIX: Use UTC to prevent timezone issues
  ...
  const handleClearWeekdays = () => {
    ...
    const year = currentCalendarDate.getFullYear();
    const month = currentCalendarDate.getMonth();
    // FIX: Use UTC functions to prevent timezone issues
    const updatedAttendance = (currentAttendanceData || []).filter(entry => {
        const entryDate = new Date(entry.date + 'T00:00:00Z'); // Parse as UTC to be safe
  ...
  const handleAttendanceTypeButtonClick = (type: AttendanceEntry['type'] | 'clear' | 'fill_5_2' | 'clear_all_month') => {
    ...
    if (type === 'clear_all_month') { setIsClearAllMonthConfirmOpen(true); return; }
    ...
    if (multiSelectedDates.size > 0) datesToUpdate.push(...Array.from(multiSelectedDates));
    else if (selectedCalendarDateForEdit) datesToUpdate.push(selectedCalendarDateForEdit);

    if (datesToUpdate.length === 0 && type !== 'clear') return;

    if (type === 'clear' && multiSelectedDates.size > 0) { setIsClearMultiSelectedConfirmOpen(true); return; }
  ...
  const handleConfirmClearAllMonth = () => {
  ...
  const handleConfirmFill52 = (overwrite: boolean) => {
    if (pendingFillTypeFor52) {
        handleBulkFillWeekdays(pendingFillTypeFor52, overwrite);
    }
    setIsFill52ConfirmOpen(false); setPendingFillTypeFor52(null);
  };

  const handleConfirmClearMultiSelected = () => {
    if (multiSelectedDates.size > 0) {
        onAttendanceChange((currentAttendanceData || []).filter(entry => !multiSelectedDates.has(entry.date)));
    }
    setIsClearMultiSelectedConfirmOpen(false); setMultiSelectedDates(new Set()); setShiftSelectionAnchorDate(null); setSelectedCalendarDateForEdit(null);
  };
  
  const handleAttendanceNotesChange = (dateStr: string, notes: string) => {
    if (!canAdminEdit) return;
  ...
  const renderCalendar = () => {
    ...
    // FIX: Explicitly type [typeKey, label] to prevent `label` from being `unknown`.
    const attendanceButtonsConfig = [
        ...Object.entries(ATTENDANCE_TYPE_LABELS).map(([typeKey, label]: [string, string]) => ({
            id: typeKey as AttendanceEntry['type'], label, action: () => handleAttendanceTypeButtonClick(typeKey as AttendanceEntry['type']),
            style: `${ATTENDANCE_TYPE_COLORS[typeKey as AttendanceEntry['type']].pill} hover:opacity-80 hover:ring-1 hover:ring-current`, isTypeButton: true,
        })),
  ...
  return (
    <div>
      {renderCalendar()}
      {isClearAllMonthConfirmOpen && (
        <ConfirmationModal isOpen={isClearAllMonthConfirmOpen} onClose={() => setIsClearAllMonthConfirmOpen(false)} onConfirm={handleConfirmClearAllMonth} title="Очистить весь месяц?" message={<p>Вы уверены, что хотите удалить все отметки о посещаемости за <strong className="text-brand-text-primary">{MONTH_NAMES[currentCalendarDate.getMonth()]} {currentCalendarDate.getFullYear()}</strong>? Это действие необратимо.</p>} confirmText="Да, очистить" confirmButtonVariant="danger" />
      )}
      {isFill52ConfirmOpen && (
        <ConfirmationModal
          isOpen={isFill52ConfirmOpen}
          onClose={() => { setIsFill52ConfirmOpen(false); setPendingFillTypeFor52(null); }}
          onConfirm={() => handleConfirmFill52(false)} // Fill without overwrite
          title="Заполнить будние дни?"
          message={<p>Заполнить все будние дни типом "<strong className="text-sky-300">{ATTENDANCE_TYPE_LABELS[pendingFillTypeFor52!] || ''}</strong>" за <strong className="text-brand-text-primary">{MONTH_NAMES[currentCalendarDate.getMonth()]} {currentCalendarDate.getFullYear()}</strong>?</p>}
          confirmText="Да (без перезаписи)"
          confirmButtonVariant="primary"
          confirmSecondaryText="Да (с перезаписью)" // New button
          onConfirmSecondary={() => handleConfirmFill52(true)} // Fill with overwrite
          confirmSecondaryButtonVariant="danger"
        />
      )}
      {isClearMultiSelectedConfirmOpen && (
        <ConfirmationModal isOpen={isClearMultiSelectedConfirmOpen} onClose={() => setIsClearMultiSelectedConfirmOpen(false)} onConfirm={handleConfirmClearMultiSelected} title="Очистить выбранные дни?" message={<p>Вы уверены, что хотите очистить отметки о посещаемости для <strong className="text-brand-text-primary">{multiSelectedDates.size} выбранных дней</strong>?</p>} confirmText="Да, очистить" confirmButtonVariant="danger" />
      )}
    </div>
  );
};

export default UserProfileAttendanceTab;