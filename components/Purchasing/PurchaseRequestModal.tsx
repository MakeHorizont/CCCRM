...
import { PurchaseRequest, PurchaseRequestStatus, User, WarehouseItem, Order as SalesOrder, Contact, HouseholdItem, PurchaseRequestItem, ProductionOrder } from '../../types';
import { TrashIcon, PlusCircleIcon, InformationCircleIcon, PencilSquareIcon } from '../UI/Icons';
...
const PurchaseRequestModal: React.FC<PurchaseRequestModalProps> = ({
  ...
  const handleItemQtyChange = (itemId: string, field: 'quantityNeeded', value: string) => {
    ...
    setRequestData(prev => ({
        ...prev,
        // FIX: Explicitly type `item` to avoid inference issues.
  ...
  const handleReceiveSubmit = async () => {
    ...
    try {
        const itemsPayload = Object.entries(editedReceivedQuantities).map(([itemId, qtyStr]) => ({
            itemId: itemId,
            newReceivedQty: parseFloat(qtyStr) || 0,
        }));
