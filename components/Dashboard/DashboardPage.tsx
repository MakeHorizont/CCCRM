...
import { Order, OrderStatus, ProductionOrder, KanbanTaskStatus, LowStockItem } from '../../types';
...
const DashboardPage: React.FC = () => {
  ...
  useEffect(() => {
    const fetchDashboardData = async () => {
      ...
      try {
        const [summary, kanbanData, lowStockData] = await Promise.all([
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 gap-6">
        {isLoadingKanban ? (
          <Card className="flex items-center justify-center p-8 h-48"><LoadingSpinner/></Card>
        ) : kanbanSummary ? (
          <KanbanSummaryWidget summary={kanbanSummary} />
        ) : (
          <Card className="text-center p-8 text-brand-text-muted h-48 flex items-center justify-center">Не удалось загрузить сводку по Kanban.</Card>
        )}
      </div>
