...
import { CircleStackIcon, PencilIcon, CheckIcon, XMarkIcon, PlusCircleIcon, TrashIcon } from '../UI/Icons';
...
const CollectiveFundOverviewPage: React.FC = () => {
    ...
    const handleSave = async () => {
        ...
        setError(null);
        try {
            const updatedFund = await apiService.updateCollectiveFund(editedData);
            setFundData(updatedFund);
            setIsEditing(false);
        } catch (err) {
            setError('Ошибка сохранения данных фонда.');
        } finally {
            setIsSaving(false);
        }
    };
                          ...
                          {canEdit && (
                            isEditing ? (
                                <div className="flex space-x-1">
                                    ...
                                    <Button size="sm" variant="primary" onClick={handleSave} isLoading={isSaving}><CheckIcon className="h-5 w-5"/></Button>
                                </div>
                            ) : (
                                ...
                                fundData.history.map(tx => (
                                    <tr key={tx.id} className="hover:bg-brand-secondary">
                                        ...
                                        <td className="px-4 py-3">
                                            <span className={`px-2 py-1 text-xs rounded-full ${tx.type === 'contribution' ? 'bg-emerald-500/20 text-emerald-300' : 'bg-red-500/20 text-red-300'}`}>
