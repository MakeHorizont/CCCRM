...
import {
    ...
    ChartPieIcon, ShieldCheckIcon, WrenchIcon, CalendarDaysIcon, BoltIcon
...
export const MENU_ITEMS: MenuItem[] = [
  ...
  { name: 'Единый Календарь', path: ROUTE_PATHS.CALENDAR, icon: CalendarDaysIcon, iconName: 'CalendarDaysIcon' },
  ...
  { name: 'Бережливое Пр-во', path: ROUTE_PATHS.LEAN, icon: BoltIcon, iconName: 'BoltIcon' }, // New Item
...
const ensureCoreItems = (config: MobileNavItemConfig[]): MobileNavItemConfig[] => {
    const corePaths = [
        ...
        ROUTE_PATHS.EQUIPMENT, ROUTE_PATHS.MAINTENANCE, ROUTE_PATHS.LEAN
    ];
    let result = [...config];
